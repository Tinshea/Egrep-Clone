  ...
  public static void main(String arg[]) {
    ...
    try {
      ...
      RegExTree ret = parse();
        ...
        List<String> lignesAcceptees = new ArrayList<>();
        if (ret.isTreeConcat()) {
            KMP kmp = new KMP();
            try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                String ligne;
                while ((ligne = br.readLine()) != null) {
                    if (kmp.accept(regEx, ligne)) {
                        lignesAcceptees.add(ligne);
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
            ...
            NFA automate = NFA.build(ret);
            automate.toDotFile("nfa.dot");
            DFA dfa = new DFA(automate);
            dfa.toDotFile("dfa.dot");
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                ...
                while ((ligne = br.readLine()) != null) {
                    for (int i = 0; i < ligne.length() - regExlength; i++) {
                        if (dfa.accept(ligne.substring(i, i + regExlength))) {
                            lignesAcceptees.add(ligne);
                            break;
                        }
                    }
        ...
        for (String ligne : lignesAcceptees) {
